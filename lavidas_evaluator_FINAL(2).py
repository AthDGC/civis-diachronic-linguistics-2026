...
"""
LAVIDAS EVALUATOR - Final Working Version with Fallback Support
...
- Local Ollama support (free, no API key needed)
- Automatic model switching on quota exceeded
...
class Config:
    ...
    DEFAULT_MODEL = "ollama-llama2"  # Changed to local by default
...
def check_api_keys():
    ...
    if check_ollama_available():
        ...
        if models:
            print_status(f"✓ Ollama available (Local). Models: {', '.join(models[:3])}", "success")
    ...
    if Config.CLAUDE_API_KEY:
        print_status("✓ Claude API key configured", "success")
    ...
    if Config.MISTRAL_API_KEY:
        print_status("✓ Mistral API key configured", "success")
    ...
    if not available:
        ...
        print("""
SOLUTIONS:
1. Install Ollama (FREE, LOCAL - No API key needed):
   https://ollama.ai
    ...
    def call_ai(
        ...
        if model.startswith("ollama-"):
            ...
            try:
                ...
                print_status(f"Ollama failed: {e}. Trying alternative...", "warning")
                # Fall back to Claude
                if Config.CLAUDE_API_KEY:
                ...
                # Fall back to Gemini
                    ...
                    raise Exception("All models failed. Check API keys or Ollama installation.")
...
class InteractiveCLI:
    ...
    def main_menu() -> str:
        ...
        print("5. Check Model Availability")
        print("6. Exit\n")
    ...
    def select_model() -> str:
        ...
        if Config.GEMINI_API_KEY:
            print(f"{next_idx}. Gemini Flash (Paid)")
        ...
        if Config.MISTRAL_API_KEY:
            print(f"{next_idx}. Mistral Large (Paid)")
...
def main():
    ...
    if sys.argv[1] in ("--help", "-h"):
        print("""
LAVIDAS EVALUATOR - Multi-Model Version with Fallback Support
...
MODELS SUPPORTED:
  ...
  2. Gemini Flash (Paid, Fast)
  3. Claude Sonnet (Paid, High Quality)
  4. Mistral Large (Paid, European)

SETUP - OPTION 1 (RECOMMENDED - FREE LOCAL):
  1. Install Ollama: https://ollama.ai
  2. Download model: ollama pull llama2
  3. Run this script: python lavidas_evaluator_FINAL.py

SETUP - OPTION 2 (Paid API Keys):
  $env:GEMINI_API_KEY = "your-key"
  $env:CLAUDE_API_KEY = "your-key"
  $env:MISTRAL_API_KEY = "your-key"
  python lavidas_evaluator_FINAL.py
