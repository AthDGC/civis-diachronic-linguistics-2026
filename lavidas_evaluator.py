...
class Config:
    ...
    DEFAULT_SECTIONS = [
        "Grammatiki & Syntaxi / Grammar & Syntax",
        "Domi & Organosi / Structure & Organization",
        "Epicheirematologia & Logiki / Argumentation & Logic",
        "Chrisi Pigon & Tekmiriosi / Use of Sources & Evidence"
    ]
...
2. Structure: Use these sections:
{sections_text}
3. Evidence: Quote specific passages from student text
...
class InteractiveCLI:
    ...
    def grade_essays_menu():
        ...
        print("\nAI Model:")
        print("1. Gemini Flash (Fastest, Recommended)")
        print("2. Gemini Pro (Best Quality)")
        print("3. Mistral Large (European)")
        model_choice = input("Choice (1-3) [1]: ").strip() or "1"
        model = ["gemini-flash", "gemini-pro", "mistral"][int(model_choice) - 1]
...
OPTIONS:
  ...
  --model flash|pro|mistral AI model
...
EXAMPLES:
  ...
  python lavidas_evaluator.py ./essays --model mistral --output results.txt
""")
            return
        ...
        while i < len(sys.argv):
            if sys.argv[i] == '--topic' and i + 1 < len(sys.argv):
                ...
                model = "gemini-flash" if m == "flash" else \
                        "gemini-pro" if m == "pro" else "mistral"
                i += 2
