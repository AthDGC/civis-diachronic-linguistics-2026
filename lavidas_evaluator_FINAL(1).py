...
# -*- coding: utf-8 -*-
"""
LAVIDAS EVALUATOR - Final Working Version
...
# CONFIGURATION
class Config:
    # YOUR WORKING API KEYS - Updated with your actual keys
    GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "AIzaSyDk13woM5df66jQ2PwkcBgAaOoDqnMOnwo")
    MISTRAL_API_KEY = os.getenv("MISTRAL_API_KEY", "SyFYmEijPBbZr5J7lw4teB794MIiRQ9o")
...
class AIModel:
    ...
    def call_gemini(api_key: str, prompt: str, temperature: float = 0.4, 
        ...
        model_name = "gemini-1.5-flash" if model == 'flash' else "gemini-1.5-pro"
    ...
    def call_ai(cls, prompt: str, model: str = "gemini-flash", 
        ...
        if model == "gemini-flash":
            if "YOUR_" in Config.GEMINI_API_KEY: raise ValueError("Gemini API Key missing")
            ...
            if "YOUR_" in Config.MISTRAL_API_KEY: raise ValueError("Mistral API key not configured")
            return cls.call_mistral(Config.MISTRAL_API_KEY, prompt, temperature)
        elif model == "claude":
            if "YOUR_" in Config.CLAUDE_API_KEY: raise ValueError("Claude API key not configured")
            return cls.call_claude(Config.CLAUDE_API_KEY, prompt, temperature)
        elif model == "perplexity":
            if "YOUR_" in Config.PERPLEXITY_API_KEY: raise ValueError("Perplexity API key not configured")
...
class InteractiveCLI:
    ...
    def grade_essays_menu():
        ...
        print("3. Claude Sonnet 3.5 (Excellent, Paid)")
    ...
    def train_style_menu():
        ...
        print("2. Gemini Pro")
        print("3. Claude Sonnet 3.5")
        model_choice = input("Choice (1-3) [1]: ").strip() or "1"
        model = ["gemini-flash", "gemini-pro", "claude"][int(model_choice) - 1]
    ...
    def generate_ade_email_menu():
        ...
        print("\nAI Model:")
        print("1. Gemini Flash (Free)")
        print("2. Gemini Pro (Free)")
        print("3. Claude Sonnet 3.5 (Excellent, Paid)")
        print("4. Mistral Large (European, Paid)")
        print("5. Perplexity (Research, Paid)")
        model_choice = input("Choice (1-5) [1]: ").strip() or "1"
        model = ["gemini-flash", "gemini-pro", "claude", "mistral", "perplexity"][int(model_choice) - 1]
    ...
    def configure_settings():
        ...
        print(f"  Default Person: {Config.DEFAULT_PERSON}")
        print(f"  Default Eval Type: {Config.DEFAULT_EVAL_TYPE}")
        print()
    ...
    @staticmethod
    def run():
        if "YOUR_" in Config.GEMINI_API_KEY:
...
OPTIONS:
  ...
  --person singular|impersonal        Voice
...
MODELS:
  ...
  claude        Claude Sonnet (excellent, paid)
